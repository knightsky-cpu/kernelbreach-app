...
function renderMenu(state2) {
  ...
  rows.push(divider(W));
  rows.push(`\u2502 ${bold("Player:")} ${color(player.name, BRIGHT_WHITE)} ${dim(`#${player.id}`)}${" ".repeat(W - 22 - player.name.length - player.id.length)} \u2502`);
  rows.push(`\u2502 ${bold("Credits:")}   ${color(`${player.gold}CR`, YELLOW)}${" ".repeat(W - 13 - player.gold.toString().length)} \u2502`);
  rows.push(`\u2502 ${bold("Bosses:")} ${color(`${player.defeatedBosses.length}/${TOTAL_BOSSES}`, player.defeatedBosses.length === TOTAL_BOSSES ? BRIGHT_YELLOW : WHITE)}${" ".repeat(W - 13)} \u2502`);
  rows.push(divider(W));
  rows.push(`\u2502 ${bold("Objective:")} ${pad(objective ? objective.bossName : "All daemons purged", W - 15)} \u2502`);
  rows.push(`\u2502 ${bold("Location:")} ${pad(objective ? objective.zoneName : "Root Directory", W - 15)} \u2502`);
  rows.push(`\u2502 ${bold("Vector:")}   ${pad(objective ? getTravelVectorLabel(player.currentZone, objective.zone) : "Center", W - 15)} \u2502`);
  rows.push(divider(W));
...
function renderInventory(player, cursor) {
  ...
  rows.push(`\u2502 ${bold("Credits:")} ${color(`${player.gold}CR`, YELLOW)}${" ".repeat(W - 12 - player.gold.toString().length)} \u2502`);
  rows.push(divider(W));
...
function renderPartyView(state2) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (!c) {
      rows.push(`\u2502 ${selected ? color("\u25B6 ", BRIGHT_YELLOW) : "  "}${dim("(empty slot)")}${" ".repeat(W - 16)} \u2502`);
      continue;
    }
    ...
    rows.push(`\u2502 ${selected ? color("\u25B6 ", BRIGHT_YELLOW) : "  "}${rColor}${name}${RESET}${sp}${reorderTag}  ${dim(`Lv${c.level} ${speciesDisplay}`)}${" ".repeat(Math.max(0, W - 20 - name.length - c.level.toString().length - speciesDisplay.length - stripAnsi(sp).length - stripAnsi(reorderTag).length))} \u2502`);
    rows.push(`\u2502    HP: ${hpBar(c.currentHp, c.maxHp, 12)} ${c.currentHp}/${c.maxHp}  XP: ${xpBar(c.xp, c.xpToNext, 8)}${" ".repeat(6)} \u2502`);
    const str = c.baseStr + c.allocatedStr;
    const con = c.baseCon + c.allocatedCon;
    rows.push(`\u2502    ATK:${calcAttack(c.level, c.baseStr, c.allocatedStr)} DEF:${calcDefense(c.level, c.baseCon, c.allocatedCon)}  STR:${str}(+${c.allocatedStr}) CON:${con}(+${c.allocatedCon})  ${RARITY_STARS[c.rarity]}${" ".repeat(8)} \u2502`);
    if (selected && c.skillPoints > 0) {
      rows.push(`\u2502    ${color("[Enter] Spend skill points", BRIGHT_CYAN)}${" ".repeat(W - 32)} \u2502`);
    }
  }
  rows.push(bottomBorder(W));
...
function renderStatUpgrade(creature, cursor) {
  ...
  rows.push(`\u2502 ${dim(`Skill Points Available: `)}${bold(color(creature.skillPoints.toString(), BRIGHT_YELLOW))}${" ".repeat(W - 30)} \u2502`);
  rows.push(divider(W));
  ...
  for (let i = 0; i < opts.length; i++) {
    ...
    const line = selected ? bold(color(`\u25B6 ${opts[i]}`, BRIGHT_YELLOW)) : dim(`  ${opts[i]}`);
    rows.push(`\u2502 ${pad(line, W - 4)} \u2502`);
    rows.push(`\u2502    ${val}${" ".repeat(Math.max(0, W - 4 - stripAnsi(val).length - 4))} \u2502`);
  }
  rows.push(bottomBorder(W));
...
function renderStorageView(state2) {
  ...
  if (player.storage.length === 0) {
    rows.push(`\u2502 ${dim("No exploits stored in Exploit Storage.")}${" ".repeat(W - 41)} \u2502`);
  } else {
    ...
    for (let i = 0; i < visible.length; i++) {
      ...
      const name = c.shiny ? `\u2726${c.nickname}` : c.nickname;
      const speciesDisplay = SPECIES_NAMES[c.species] ?? c.species;
      rows.push(`\u2502 ${selected ? color("\u25B6 ", BRIGHT_YELLOW) : "  "}${rColor}${name.padEnd(14)}${RESET} ${dim(`Lv${c.level} ${speciesDisplay.padEnd(12)} ${RARITY_STARS[c.rarity]}`)}${" ".repeat(4)} \u2502`);
    }
  }
  rows.push(bottomBorder(W));
...
function renderBattle(state2) {
  ...
  pushRow(`\u2502${enemyName}${combatGap}${playerName}\u2502`);
  pushRow(`\u2502${enemyBar}${combatGap}${playerBar}\u2502`);
  ...
  pushRow(`\u2502${statusLeft}${combatGap}${statusRight}\u2502`);
  pushRow(divider(W));
  ...
  if (battle.phase === "player_action") {
    if (battle.selectingSkill) {
      ...
      pushRow(`\u2502 ${skillStr}${" ".repeat(Math.max(0, W - 2 - stripAnsi(skillStr).length))} \u2502`);
      ...
      pushRow(`\u2502 ${scriptStr}${" ".repeat(Math.max(0, W - 2 - stripAnsi(scriptStr).length))} \u2502`);
      ...
      pushRow(`\u2502 ${switchStr}${" ".repeat(Math.max(0, W - 2 - stripAnsi(switchStr).length))} \u2502`);
      ...
      pushRow(`\u2502 ${actionStr}${" ".repeat(Math.max(0, W - 2 - stripAnsi(actionStr).length))} \u2502`);
